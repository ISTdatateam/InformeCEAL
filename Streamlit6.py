...
# Asegúrate de contar con este archivo y haber definido
# las variables: DB_SERVER, DB_DATABASE, DB_USERNAME y DB_PASSWORD
load_dotenv()

# Configuración de logging para monitorear la aplicación.
# Esto ayuda a registrar eventos y errores para diagnósticos.
logging.basicConfig(
    level=logging.INFO,
...
# Ajustamos el título y el layout
st.set_page_config(
...
def consultar_tabla(tabla, cuv, columnas=None):
    ...
    query = f"SELECT {columnas_sql} FROM {tabla} WHERE CUV = ?"
...
def consultar_tabla_sincuv(tabla):
    ...
    if connection:
        try:
            ...
            logging.info(f"Consulta ejecutada en la tabla '{tabla}'")
            return df
        except Exception as e:
            st.error(f"Error al consultar la tabla '{tabla}': {e}")
            logging.error(f"Error al consultar la tabla '{tabla}': {e}")
...
if 'df_res_com' not in st.session_state:
    st.session_state.df_res_com = pd.DataFrame()
...
if st.button("Buscar"):
    if cuv_valor.strip() == "":
        ...
        resultados["df_ciiu"] = consultar_tabla_sincuv("informe_CEAL__ciiu")
        resultados["df_recomendaciones"] = consultar_tabla_sincuv("informe_CEAL__rec")
        ...
        if not st.session_state.get("df_resultados", pd.DataFrame()).empty:
        ...
        otras_tablas = {
            "Base Completa": st.session_state.df_res_com,
...
# Nota de seguridad en la barra lateral
st.sidebar.info(
    """
    **Nota de Seguridad:**
    Las credenciales de la base de datos están manejadas de forma segura mediante variables de entorno.
    Asegúrate de configurar correctamente el archivo `.env` y de no compartir tus credenciales.
    """
)
...
def mostrar_datos(datos, estado):
    ...
    **Fecha de activación del cuestionario:** {datos.get('Fecha_Inicio', 'N/A')}  
    **Fecha de cierre del cuestionario:** {datos.get('Fecha_Fin', 'N/A')}  
...
if not summary_df.empty:
    cuv_list = summary_df['CUV'].unique()
    # Si hay al menos un valor, lo toma automáticamente
    if len(cuv_list) > 0:
        selected_cuv = cuv_list[0]
    else:
        selected_cuv = None
else:
    selected_cuv = None
...
fecha_fin = pd.to_datetime(datos.get('Fecha_Fin'))
...
def generar_contenido_word(datos, estado_riesgo, fig_principal, figs_te3, interpretaciones_df, summary_df, confirmadas_df, df_resultados_porcentaje):
    ...
    p.add_run(f"{datos.get('Fecha_Inicio', 'N/A')}\n")
    p.add_run('Fecha de cierre del cuestionario: ').bold = True
    p.add_run(f"{datos.get('Fecha_Fin', 'N/A')}\n")
